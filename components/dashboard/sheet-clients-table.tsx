      ...
      <div className="flex items-start gap-1.5 min-w-0 w-full">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="text-primary hover:underline truncate font-medium">
              {row.name || "—"}
            </span>
        ...
        >
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {row.name.trim() ? (
                <>
                  <span className="font-medium text-foreground">{row.name.trim()}</span> sai
                  apenas do Acompanhamento. Não vai para Arquivados nem Prospects. Financeiro e
                  Serviços e Custos permanecem.
                </>
              ) : (
                "O cliente sai apenas do Acompanhamento. Não vai para Arquivados nem Prospects. Financeiro e Serviços e Custos permanecem."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function SheetClientsTable({
  ...
  const filteredRows = useMemo(() => {
    ...
    const matched = term
      ? rows.filter((row) => {
          if (row.group?.toLowerCase().includes(term)) {
            return true;
          }
